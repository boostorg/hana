...
void test() {
    BOOST_HANA_STATIC_ASSERT(metafunction<f>() == type<typename f<>::type>);
    BOOST_HANA_STATIC_ASSERT(metafunction<f>(type<x1>) == type<typename f<x1>::type>);
    BOOST_HANA_STATIC_ASSERT(metafunction<f>(type<x1>, type<x2>) == type<typename f<x1, x2>::type>);
    BOOST_HANA_STATIC_ASSERT(metafunction<f>(type<x1>, type<x2>, type<x3>) == type<typename f<x1, x2, x3>::type>);
...
int main() {
    ...
    // BOOST_HANA_STATIC_ASSERT(!detail::is_valid(metafunction<invalid>)(type<x1>));
    // test<alias>();
}
