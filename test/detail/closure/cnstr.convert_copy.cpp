...
int main() {
    ...
    {
        using T0 = detail::closure<double>;
        using T1 = detail::closure<A>;
        constexpr T0 t0(2.5);
        constexpr T1 t1 = t0;
        static_assert(detail::get<0>(t1) == 2, "");
    }
    {
        using T0 = detail::closure<int>;
        using T1 = detail::closure<C>;
        constexpr T0 t0(2);
        constexpr T1 t1{t0};
        static_assert(detail::get<0>(t1) == C(2), "");
    }
    {
    ...
    {
        using T0 = detail::closure<double, char, int>;
        using T1 = detail::closure<int, int, B>;
        T0 t0(2.5, 'a', 3);
        T1 t1(t0);
        BOOST_HANA_RUNTIME_CHECK(detail::get<0>(t1) == 2);
