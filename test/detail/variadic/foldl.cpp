...
void test_impl(S s, R result) {
    BOOST_HANA_CONSTANT_CHECK(equal(variadic::foldl_t<Ts...>(mf, s), result));
    BOOST_HANA_CONSTANT_CHECK(equal(variadic::foldl_t<Ts...>(f, s), result));
    BOOST_HANA_CONSTANT_CHECK(equal(variadic::foldl(mf, s, type<Ts>...), result));
    BOOST_HANA_CONSTANT_CHECK(equal(variadic::foldl(f, s, type<Ts>...), result));
}
