...
void test(R result) {
    ...
    BOOST_HANA_CONSTANT_CHECK(equal(variadic::foldl1_t<Ts...>(mf), result));
    BOOST_HANA_CONSTANT_CHECK(equal(variadic::foldl1_t<Ts...>(f), result));
    BOOST_HANA_CONSTANT_CHECK(equal(variadic::foldl1(mf, type<Ts>...), result));
    BOOST_HANA_CONSTANT_CHECK(equal(variadic::foldl1(f, type<Ts>...), result));
}
