...
int main() {
    ...
    // List
    {
        ...
        {
            BOOST_HANA_CONSTANT_CHECK(equal(
                cons(int_<0>, integer_list<long>),
                integer_list<long, 0>
            ));
            BOOST_HANA_CONSTANT_CHECK(equal(
                cons(int_<0>, integer_list<unsigned int, 1>),
                integer_list<unsigned int, 0, 1>
            ));
            BOOST_HANA_CONSTANT_CHECK(equal(
                cons(long_<0>, integer_list<long long, 1, 2>),
                integer_list<long long, 0, 1, 2>
            ));
            BOOST_HANA_CONSTANT_CHECK(equal(
                cons(ulong<0>, integer_list<unsigned long, 1, 2, 3>),
