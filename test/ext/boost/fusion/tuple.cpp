...
int main() {
    ...
    {
        BOOST_HANA_CONSTANT_CHECK(hana::equal(
            hana::drop_front_exactly(fusion::make_tuple(ct_eq<0>{})),
            fusion::make_tuple()
        ));

        BOOST_HANA_CONSTANT_CHECK(hana::equal(
            hana::drop_front_exactly(fusion::make_tuple(ct_eq<0>{}, ct_eq<1>{})),
            fusion::make_tuple(ct_eq<1>{})
        ));

        BOOST_HANA_CONSTANT_CHECK(hana::equal(
            hana::drop_front_exactly(fusion::make_tuple(ct_eq<0>{}, ct_eq<1>{}, ct_eq<2>{})),
            fusion::make_tuple(ct_eq<1>{}, ct_eq<2>{})
        ));

        BOOST_HANA_CONSTANT_CHECK(hana::equal(
            hana::drop_front_exactly(fusion::make_tuple(ct_eq<0>{}, ct_eq<1>{}, ct_eq<2>{}, ct_eq<3>{})),
            fusion::make_tuple(ct_eq<1>{}, ct_eq<2>{}, ct_eq<3>{})
        ));


        BOOST_HANA_CONSTANT_CHECK(hana::equal(
            hana::drop_front_exactly(fusion::make_tuple(ct_eq<0>{}, ct_eq<1>{}, ct_eq<2>{}), hana::size_c<2>),
            fusion::make_tuple(ct_eq<2>{})
        ));

        BOOST_HANA_CONSTANT_CHECK(hana::equal(
            hana::drop_front_exactly(fusion::make_tuple(ct_eq<0>{}, ct_eq<1>{}, ct_eq<2>{}, ct_eq<3>{}), hana::size_c<2>),
            fusion::make_tuple(ct_eq<2>{}, ct_eq<3>{})
        ));

        BOOST_HANA_CONSTANT_CHECK(hana::equal(
            hana::drop_front_exactly(fusion::make_tuple(ct_eq<0>{}, ct_eq<1>{}, ct_eq<2>{}, ct_eq<3>{}), hana::size_c<3>),
            fusion::make_tuple(ct_eq<3>{})
        ));
    }
