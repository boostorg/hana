...
int main() {
    // unpack
    {
        BOOST_HANA_CONSTANT_ASSERT(equal(unpack(range(int_<0>, int_<0>), f), f()));
        BOOST_HANA_CONSTANT_ASSERT(equal(unpack(range(int_<0>, int_<1>), f), f(int_<0>)));
        BOOST_HANA_CONSTANT_ASSERT(equal(unpack(range(int_<0>, int_<2>), f), f(int_<0>, int_<1>)));
        BOOST_HANA_CONSTANT_ASSERT(equal(unpack(range(int_<0>, int_<3>), f), f(int_<0>, int_<1>, int_<2>)));
    }
