...
template <typename f>
void test() {
    BOOST_HANA_CONSTANT_CHECK(metafunction_class<f>() == type<typename f::template apply<>::type>);
    BOOST_HANA_CONSTANT_CHECK(metafunction_class<f>(type<x1>) == type<typename f::template apply<x1>::type>);
    BOOST_HANA_CONSTANT_CHECK(metafunction_class<f>(type<x1>, type<x2>) == type<typename f::template apply<x1, x2>::type>);
    BOOST_HANA_CONSTANT_CHECK(metafunction_class<f>(type<x1>, type<x2>, type<x3>) == type<typename f::template apply<x1, x2, x3>::type>);
...
int main() {
    ...
    // test<alias>();
    // BOOST_HANA_CONSTANT_CHECK(!detail::is_valid(metafunction_class<invalid>)(type<x1>));
}
