...
void test() {
    ...
    BOOST_HANA_CONSTANT_ASSERT(unfoldr<L>(stop_at(x<0>), x<0>) == list());
    BOOST_HANA_CONSTEXPR_ASSERT(unfoldr<L>(stop_at(x<1>), x<0>) == list(f(x<0>)));
    BOOST_HANA_CONSTEXPR_ASSERT(unfoldr<L>(stop_at(x<2>), x<0>) == list(f(x<0>), f(x<1>)));
    BOOST_HANA_CONSTEXPR_ASSERT(unfoldr<L>(stop_at(x<3>), x<0>) == list(f(x<0>), f(x<1>), f(x<2>)));
    BOOST_HANA_CONSTEXPR_ASSERT(unfoldr<L>(stop_at(x<4>), x<0>) == list(f(x<0>), f(x<1>), f(x<2>), f(x<3>)));
}
...
void test_revert() {
    ...
    for_each(lists, [=](auto xs) {
        BOOST_HANA_CONSTANT_ASSERT(unfoldr<L>(g, foldr(xs, z, f)) == xs);
    });
}
