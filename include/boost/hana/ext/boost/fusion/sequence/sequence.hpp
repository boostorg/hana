/*!
@file
Forward declares `boost::hana::BoostFusionSequence`.
...
#ifndef BOOST_HANA_EXT_BOOST_FUSION_SEQUENCE_SEQUENCE_HPP
#define BOOST_HANA_EXT_BOOST_FUSION_SEQUENCE_SEQUENCE_HPP
...
namespace boost { namespace hana {
    ...
    struct BoostFusionSequence;

    template <typename T>
    struct datatype<T, when<
        fusion_detail::is_nonassociative_forward_sequence<T>::value
    >> {
        using type = BoostFusionSequence;
    };
}} // end namespace boost::hana

#endif // !BOOST_HANA_EXT_BOOST_FUSION_SEQUENCE_SEQUENCE_HPP
