...
namespace boost { namespace hana {
    ...
    struct Comparable::instance<Set, Set> : Comparable::equal_mcd {
        ...
        static constexpr auto equal_impl(S1 s1, S2 s2) {
            return and_(
                ...
                all([=](auto k) {
                    return equal(find_key(k, s1), find_key(k, s2));
                }, detail::unwrap(s1))
            );
        }
    };
    ...
    template <typename L>
    struct convert<Set, L, detail::enable_if_t<instantiates<List, L>()>> {
    ...
    //! @relates Set
    template <typename L>
    struct convert<L, Set, detail::enable_if_t<instantiates<List, L>()>> {
